...
class TestAdvancedBoots(unittest.TestCase):
    ...
    def test_annotations_are_not_mods(self):
        ...
        self.assertEqual(len(self.item.mods), 7)

    def test_rune_mod_keeps_its_kind(self):
        rune = next(m for m in self.item.mods if m.kind == "rune")
        self.assertEqual(rune.text, "5% increased Movement Speed")
        self.assertIsNone(rune.tier)
        # одноимённый аффикс предмета руной не считается
        explicit_ms = [
            m for m in self.item.mods if "Movement Speed" in m.text and m.kind == "explicit"
        ]
        self.assertEqual(len(explicit_ms), 1)
        self.assertEqual(explicit_ms[0].values, [30.0])

    def test_fractional_roll_range(self):
        regen = next(m for m in self.item.mods if "Life Regeneration" in m.text)
        self.assertEqual(regen.text, "16.1 Life Regeneration per second")
        self.assertEqual(regen.values, [16.1])
        self.assertEqual(regen.tier, 2)
    ...
    def test_roll_ranges_stripped(self):
        ...
        self.assertIn("+29% to Lightning Resistance", texts)
        self.assertIn("+36% to Fire Resistance", texts)
        self.assertIn("30% increased Movement Speed", texts)
    ...
    def test_kind_from_annotation(self):
        kinds = [m.kind for m in self.item.mods]
        self.assertEqual(kinds.count("rune"), 1)
        self.assertEqual(kinds.count("explicit"), 6)
...
class TestMatchingAfterFix(unittest.TestCase):
    ...
    def test_all_boot_mods_matched(self):
        """Все семь строк должны быть кликабельны в оверлее."""
    ...
    def test_rune_matched_but_off_by_default(self):
        """Руна опознаётся (чекбокс активен), но в поиск сразу не идёт:
        её можно переставить, к ценности самого предмета она не относится."""
        item = parse_item(ADVANCED_BOOTS)
        filters = build_mod_filters(item, self.db, default_kinds=["explicit"])
        rune = next(f for f in filters if f.kind == "rune")
        self.assertTrue(rune.matched)
        self.assertFalse(rune.enabled)
        self.assertTrue(all(f.enabled for f in filters if f.kind == "explicit"))
