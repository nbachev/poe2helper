...
class PriceCheckOverlay(QWidget):
    ...
    def _choose_from_pool(self, initial_query: str = "", title: str = "Добавить мод из пула"):
        """Открывает диалог пула и возвращает ``(entry, option)`` либо ``None``."""
        db = self.ctx.stats_db
        if db.is_empty:
            self.lbl_status.setText("Пул модов ещё не загружен (Настройки → Обновить данные).")
            return None
        dialog = AddModDialog(db, self, initial_query=initial_query, title=title)
        was_pinned = self.btn_pin.isChecked()
        self.btn_pin.setChecked(True)  # иначе оверлей спрячется, потеряв фокус
        try:
            if dialog.exec() != AddModDialog.DialogCode.Accepted or not dialog.selected_entry:
                return None
            return dialog.selected_entry, dialog.selected_option
        finally:
    ...
    def _pick_for_row(self, row: ModRow, mod: ModFilter) -> None:
        ...
        mod.stat_id = entry.id
        mod.kind = entry.kind
        mod.matched = True
        mod.enabled = True
        mod.option_id = option[0] if option else None
        if option:
            mod.text = entry.text.replace("#", option[1])
            mod.min_value = mod.max_value = None
        elif mod.source_value is not None:
            cfg_search = self.ctx.cfg.get("search") or {}
            mod.min_value, mod.max_value = suggest_bounds(
                mod.source_value,
                float(cfg_search.get("roll_tolerance", 0.1)),
                bool(cfg_search.get("use_min_only", True)),
            )
        row.refresh()
